		...
		for (Document doc : search) {
			...
			if (listEvents != null && !listEvents.isEmpty()) {
				for (Document event : listEvents) {
					data = df.parse(event.getString(EVENT_DATE));
...
//			String strDate = doc.getString(START_DATE);
			...
			if (listEvents != null && !listEvents.isEmpty()) {
				for (Document event : listEvents) { // ciclo per aumentare i record			    					
					addMapValue(numDay, event.getString(EVENT_DATE), 1);
				}
			}
		}
	...
	public static JsonObject dataAvarageTraumaTime(final FindIterable<Document> search) {
	    ...
	    final JsonObject jsonObj = createCartesianChart(avgTraumaTime);
	    jsonObj.put("media", (int)(numRep > 0 ? sumTime/numRep : 0));
...
//				String place = event.getString("place");
		...
		return createCartesianChart(timeRoom);
	}
	...
	public static JsonObject dataGetMaxProcedure(final FindIterable<Document> search) {
		...
		return createCartesianChart(maxProcedure).put("totProcedureString", 
				maxProcedure.values().size());
	}
