...
function numElemFunct() {
	...
	var numElemTo = (Math.floor((numElem / scaleMin)) + 1);
	return numElemTo;
}
...
function getBarCanvas(response){	
	dataY = response.y;
	dataLabelY = dataY;
...
function getChartCanvas(dataTitle) {
	var dim = Math.round((media / scaleMin));
	
	if (dim <= 0) {
		dim++;
	} else if (dim > scaleMid) {
		dim = Math.round(dim / scaleMid) * scaleMid;
	} else if (dim > scaleMin) {
		dim = Math.round(dim / scaleMin) * scaleMin;
	}
...
function initBarCanvas(response) {
	myChart = getBarCanvas(response);
}
