...
function remChartData() {
	if (lastIndex > 0) {
		  var numElem = dataValue.length;
		  var numCanc = (Math.floor((numElem / 10)) + 1);
		...
		alert("Impossibile rimuovere i dati");  
	}
}
...
function getChartCanvas(dataTitle, size) {
...
function initChartCanvas(response, label, num) {
	loadChartData(response);
	myChart = getChartCanvas(label, num);
}
...
$scope.closeGraph = function(){
  ...
  $scope.openGetNumTraumaMonth = function() {
	  service.getStat(numTraumiMonth).then(function(response){
		...
		initChartCanvas(response, 'Numero Interventi', 5);
  ...
  $scope.openGetAvarageTraumaTime = function(){
	  service.getStat(numAvarageTraumaTime).then(function(response){		
			...
			initChartCanvas(response, 'Numero Interventi', 500);
  ...
  $scope.openGetAvarageTimeRoom = function() {
	  service.getStat(numAvarageTimeRoom).then(function(response){		
			...
			initChartCanvas(response, 'Numero Interventi', 1500);
	  ...
	  service.getStat(consumptionTypeDrug).then(function(response){
		  ...
		  $("#addDataSet").click(function(){
			  ...
			  if (data1[0] == null) {
				  ...
				  myChart.update();
			  } else if (data1[0] != null && data2[0] == null) {
				  data2 = response.x3;
				  newDataset = {
						  	label: "second",
		  ...
		  $("#remDataSet").click(function(){
			  if (data1[0] != null && data2[0] != null) {
				  data2 = [];
  ...
  $scope.openGetMaxProcedure = function() {	  
	  service.getStat(numMaxProcedure).then(function(response){			
			...
			initChartCanvas(response, 'Numero Procedure', 1);
